...
const LOCAL_WORD_PROBLEMS: MathProblem[] = [
  ...
  { id: 'l8', type: 'word', question: "Anh cao 95cm, em thấp hơn anh 10cm. Hỏi em cao bao nhiêu xăng-ti-mét?", answer: 85, numbers: [95, 10], operators: ['-'] },
...
export const generateWordProblem = async (forcedOperator?: '+' | '-'): Promise<MathProblem> => {
  try {
    ...
    const response = await ai.models.generateContent({
      ...
      Constraints: 
      1. ${opInstruction}
      2. All numbers used in the question MUST be between 0 and 100.
      3. The final answer MUST be between 0 and 100.
      4. DO NOT generate negative results.
      5. Engaging context suitable for children.`,
      config: {
